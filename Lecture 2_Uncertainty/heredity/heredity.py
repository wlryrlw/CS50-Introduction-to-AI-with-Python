...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    def passes_gene(parent, child_gene_count):
        """
        Return probability that `parent` passes the gene to child.
        """
        parent_genes = gene_count(parent)
        mutation = PROBS["mutation"]

        # Parent has gene with probability based on count, with mutation flip
        if parent_genes == 2:
            return 1 - mutation
        if parent_genes == 1:
            return 0.5
    ...
    for person in people:
        ...
        if mother is None and father is None:
            ...
            mom_pass = passes_gene(mother, genes)
            dad_pass = passes_gene(father, genes)
        ...
        # Trait probability conditional on genes
